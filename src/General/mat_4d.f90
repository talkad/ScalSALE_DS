...
module mat_4d_module
   ...
   type, extends(data_struct_t) :: mat_4d_t
      private


      real(8), dimension(:,:,:,:), pointer, private         :: values
      real(8), dimension(:,:,:,:), pointer, private         :: mat_4d
   ...
   end interface mat_4d_t

contains

   function Constructor_init_arr(initial_data, d1, d2, d3, d4)
      implicit none
      type(mat_4d_t), pointer :: Constructor_init_arr
      ...
      allocate (Constructor_init_arr%values (1:1, 0:1, 0:1, 0:1))
      allocate(Constructor_init_arr%mat_4d(1:d4, 0:d1, 0:d2, 0:d3))
      Constructor_init_arr%mat_4d =  initial_data
   ...
   function Constructor_init_val(initial_val, d1, d2, d3, d4)
      implicit none
      type(mat_4d_t), pointer  ::  Constructor_init_val
      ...
      allocate (Constructor_init_val%values (1:1, 0:1, 0:1, 0:1))
      allocate(Constructor_init_val%mat_4d(1:d4, 0:d1, 0:d2, 0:d3))
      Constructor_init_val%mat_4d = initial_val
   ...
   Constructor_init_val%nmats = d4
    ...
    pure subroutine add_item(this, material_type, i, j, k, val, boundry)
        implicit none
        class(mat_4d_t), intent(inout) :: this
        integer, intent(in) :: i, j, k, material_type
        real(8), intent(in) :: val
        logical, optional, intent(in) ::  boundry
 
         this%mat_4d(material_type, i, j, k) = val
         ! write(7899, *) material_type, i, j, k, this%get_item(material_type, i, j, k)
    end subroutine add_item


    function get_item(this, material_type, i, j, k)
        implicit none
        class(mat_4d_t), intent(in) :: this
        integer, intent(in) :: i, j, k, material_type
        integer :: i_new, j_new, k_new
        real(8) :: get_item
        
        get_item = this%mat_4d(material_type, i, j, k)
      !   write(4567, *) material_type, i, j, k, get_item
    ...
    subroutine print_data(this, file_name)
         ...
         do k = 0, this%nz
               do j = 0, this%ny
                  do i = 0, this%nx
                     do m = 1, this%nmats
                        write(414,*)   m, i, j, k, this%mat_4d(m,i,j,k)                        
                     end do
                  end do
               end do
         end do
   ...
   subroutine deallocate_data(this)
      ...
      if (associated(this%mat_4d))   deallocate(this%mat_4d)
   ...
   subroutine Ptr_coordinates_4d_data (this, ptr)
      class (mat_4d_t)                    , intent(in out)  :: this
