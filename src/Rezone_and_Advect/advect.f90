...
contains
        ...
        do ii = 1, this%n_materials
            ...
            if (numb < num_mat_cell(itd, jtd, k_donner)) then
                ...
                this%adv_mats%fxtm(ii) = sg * min(abs(this%adv_mats%fxtm(ii)), &
                    0.5 * cell_mass_vof%get_item(ii, id,jd,kd) / (density_vof%get_item(ii, id,jd,kd) + 1d-20))
                ...
                fxtmt = sg * min(abs(this%adv_mats%fxtm(ii)), 0.5*cell_mass_vof%get_item(ii, id, jd, kd) / (density_vof%get_item(ii, id, jd, kd) + 1d-20))
    ...
    subroutine Calculate_advect_3d(this)
        ...
        ! write(*,*) this%nzp, this%nyp, this%nxp
        do k = 0, this%nzp
            do j = 0, this%nyp
                do i = 0, this%nxp
                    do tmp_mat = 1, this%n_materials
                        ...
                        ! write(1245, *) 'fuck me:', tmp_mat, i, j, k, init_mat_layers_adv(tmp_mat, i, j, k), mat_cell_mass_adv  (tmp_mat, i, j, k), sie_vof_adv        (tmp_mat, i, j, k)
                    end do
                end do
            end do
        end do
                    ...
                    do face = 1, 6
                        ...
                        if (abs(hexa_vol_quart) > 1d-15) then
                            ...
                            do tmp_mat = 1, this%n_materials
                                ...
                                ! write(4141, *), tmp_mat, id, jd, kd, density_vof%get_item(tmp_mat, id, jd, kd), dvof, sie_vof%get_item(tmp_mat, id, jd, kd)
                ...
                do i = 1, this%nx
                    do tmp_mat=1,this%n_materials
                        ...
                        if (mat_vof%get_item(tmp_mat,i, j, k) > this%emf .and. mat_cell_mass_adv(tmp_mat, i, j, k) > this%emfm) then
                            ! write(3131, *) "asd:", tmp_mat, i, j, k, sie_vof_adv(tmp_mat,i, j, k) , mat_cell_mass_adv(tmp_mat,i, j, k)
                            ...
                            ! write(3131, *) "asd:", tmp_mat, i, j, k, 0d0
                ...
                do i = 1, this%nx
                    do tmp_mat=1,this%n_materials
                        ...
                        if (mat_vof%get_item(tmp_mat,i, j, k) < this%emf .or. mat_cell_mass_adv(tmp_mat,i, j, k) < this%emfm) then
                            ...
                            call sie_vof%add_item(tmp_mat,i, j, k, 0d0)
                            ! write(2471, *) "advect sie_vof:", i,j,k,tmp_mat,  0d0
                            call mat_vof%add_item(tmp_mat,i, j, k, 0d0)
                        else
                            ...
                            ! write(2471, *) "advect sie_vof:", i,j,k,tmp_mat,  sie_vof%get_item(tmp_mat,i, j, k), cell_mass_vof%get_item(tmp_mat,i, j, k)
