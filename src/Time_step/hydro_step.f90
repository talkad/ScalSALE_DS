...
contains
    ...
    type(hydro_step_t) function Constructor (df, nx, ny, nz, nxp, nyp, nzp, wilkins_scheme, mesh, velocity, acceleration,&
        ...
        ! print*, 'hello there', nz, ny, nx, nmats
        ! call debug(materials%vof%data_4d%nz_values, 'material_results/vof2.5.txt', nz, ny, nx, nmats)
        ...
        ! call debug(materials%vof%data_4d%nz_values, 'material_results/vof4.txt', Constructor%nz, Constructor%ny, Constructor%nx, Constructor%nmats)   
        ...
        ! call debug(materials%vof%data_4d%nz_values, 'material_results/vof5.txt', Constructor%nz, Constructor%ny, Constructor%nx, Constructor%nmats)   
    ...
    subroutine Calculate_thermodynamics(this)
        ...
        class(data_struct_t), pointer :: temperature_vof_old
        class(data_struct_t), pointer :: temperature_vof
        class(data_struct_t), pointer :: cell_mass_vof
        class(data_struct_t), pointer :: pressure_vof
        class(data_struct_t), pointer :: density_vof
        class(data_struct_t), pointer :: dp_drho_vof
        class(data_struct_t), pointer :: dp_de_vof
        class(data_struct_t), pointer :: dt_de_vof
        class(data_struct_t), pointer :: sie_vof
        class(data_struct_t), pointer :: mat_vof
        class(data_struct_t), pointer :: sound_vel_vof
       ...
       ! call this%inversed_vertex_mass%Exchange_virtual_space_nonblocking()
        ...
        call this%materials%dp_drho    %get_quantity_grid(dp_drho_vof)
        call this%materials%dp_de      %get_quantity_grid(dp_de_vof)
        call this%materials%cell_mass      %get_quantity_grid(cell_mass_vof)
        call this%materials%sound_vel      %get_quantity_grid(sound_vel_vof)
        call this%materials%dt_de          %get_quantity_grid(dt_de_vof)
                ...
                do i = 1, this%nx
                    do tmp_mat = 1, this%nmats
                        ...
                        if (vof(i, j, k) >= this%emf) then
                            call temperature_vof%add_item(tmp_mat,i,j,k, 0d0)
                            call pressure_vof%add_item(tmp_mat,i,j,k, 0d0)
                            call dp_drho_vof%add_item(tmp_mat,i,j,k, 0d0)
                            call dp_de_vof%add_item(tmp_mat,i,j,k, 0d0)
                        end if
                    end do
                end do
            end do
                ...
                do i = 1, this%nx
                    do tmp_mat = 1, this%nmats
                        ...
                        temperature(i, j, k) = temperature(i, j, k) + &
                            temperature_vof_old%get_item(tmp_mat,i,j,k) * cell_mass_vof%get_item(tmp_mat,i,j,k) / (dt_de_vof%get_item(tmp_mat,i,j,k) + 1d-30)
                        sound_vel  (i, j, k) = sound_vel  (i, j, k) + sound_vel_vof%get_item(tmp_mat,i,j,k) * mat_vof%get_item(tmp_mat,i,j,k)
                        pressure   (i, j, k) = pressure   (i, j, k) + pressure_vof %get_item(tmp_mat,i,j,k) * mat_vof%get_item(tmp_mat,i,j,k)
                        dp_drho    (i, j, k) = dp_drho    (i, j, k) + dp_drho_vof  %get_item(tmp_mat,i,j,k) * mat_vof%get_item(tmp_mat,i,j,k)
                        dp_de      (i, j, k) = dp_de      (i, j, k) + cell_mass_vof%get_item(tmp_mat,i,j,k) / (dp_de_vof%get_item(tmp_mat,i,j,k) + 1d-30)
                        dt_de_temp (i, j, k) = dt_de_temp (i, j, k) + cell_mass_vof%get_item(tmp_mat,i,j,k) / (dt_de_vof%get_item(tmp_mat,i,j,k) + 1d-30)
                    end do
                end do
            end do
        end do
            ...
            do j = 1, this%ny
                do i = 1, this%nx
                    do tmp_mat = 1, this%nmats
                        ...
                        if (mat_vof%get_item(tmp_mat,i,j,k) > this%emf) then
                            call density_vof%add_item(tmp_mat,i,j,k, cell_mass_vof%get_item(tmp_mat,i,j,k) / (vol(i, j, k) * mat_vof%get_item(tmp_mat,i,j,k))) 
                        end if
                ...
                do i = 1, this%nx
                    do tmp_mat = 1, this%nmats
                        ...
                        if (mat_vof%get_item(tmp_mat,i,j,k) < this%emf) then
                            ...
                            sie_vof_temp = sie_vof%get_item(tmp_mat,i,j,k)
                            ...
                            if (nmats_in_cell(i, j, k) == 1d0) then
                                ...
                                pressure_temp = pressure_vof%get_item(tmp_mat,i,j,k)
                                dp_de_temp    = dp_de_vof   %get_item(tmp_mat,i,j,k)
                                dp_drho_temp  = dp_drho_vof %get_item(tmp_mat,i,j,k)
                            end if
                            pressure_temp  = pressure_temp * vof(i, j, k)   
                            mass_vof_temp  = cell_mass_vof%get_item(tmp_mat,i,j,k)


                            vol_diff_vof_temp = vol_diff * mat_vof%get_item(tmp_mat,i,j,k)
                            vol_vof_temp  = vol(i, j, k) * mat_vof%get_item(tmp_mat,i,j,k)
                            ...
                            call sie_vof%add_item(tmp_mat,i,j,k, sie_vof%get_item(tmp_mat,i,j,k) + sie_diff)
                            sie(i, j, k) = sie(i, j, k) + sie_vof%get_item(tmp_mat,i,j,k) * mass_vof_temp
                        end if
                    end do
                end do
            end do
